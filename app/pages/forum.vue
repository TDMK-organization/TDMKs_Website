<template>
    <LayoutPageLayout class="bg_layout">
        <UPageHeader
            ...
            description="Đây là trang chia sẻ kiến thức, tài liệu, thủ thuật về công nghệ và dịch vụ mà TDMK đã biên tập được với mục đích phi lợi nhuận."
            headline="News & Forum"
            :links="links"
        />
...
const { data: postsTop, pending: pendingTop } = await useFetch(
    "/api/post/postsCategory",
    {
        method: "GET",
        query: {
            id_category: "69c9eadfb09007343f9a77e1",
            limit: 4,
        },
...
const { data: responseList, pending } = await useFetch(
    "/api/post/postsCategory",
    {
        method: "GET",
        query: {
            id_category: "69ae7e1fb09007343f9a6eea",
